...
public class OmeroIceServer extends AbstractTileableImageServer {
	...
	private class GatewayWrapper {
		...
		GatewayWrapper(URI uri) {
			gateway = new Gateway(new SimpleLogger());

			// Try to use existing web client login
			// If we can get the session UUID, then we don't need a username & password
			WebClients.createClient(uri.toString()).thenAccept(client -> Platform.runLater(() -> {
				if (client.isPresent()) {
					try {
						String userName = client.get().getSessionUUID();
						if (userName != null)
					...
					} catch (DSOutOfServiceException | IOException | ExecutionException | DSAccessException e) {
